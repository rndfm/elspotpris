...
<div
	bind:this={container}
	on:touchstart={disableAutoScroll}
	on:scroll={() => (scrollLeft = container.scrollLeft)}
	on:scroll={disableAutoScroll}
	class="scrollable"
		...
		<svg width="100%" {height} class="bars">
			...
			{#if co2EnabledData}
				{#each co2EmissionsData as emission}
					<rect
						y={height - 20 - (emission.emission / co2Max) * (height - 40)}
						x="{emission.position}%"
						height="2px"
						width="1%"
						fill="aqua"
...
<ul class="legend">
	<li><span class="dot price" />Pris</li>
	<li
		behavior="button"
		on:click={toggleCo2}
		on:keypress={toggleCo2}
		class="clickable"
		class:disabled={!co2EnabledData}
	>
		<span class="dot co2" />CO2 udledning
	</li>
</ul>
